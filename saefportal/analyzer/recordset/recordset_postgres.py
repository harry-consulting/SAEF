...
class RecordsetPostgres(Recordset):
    ...
    def get_column_types(self):
        self._datastore.execute_query(
            f"create view public.tmp_1 as {self._query}")
        result = self._datastore.fetch_all(f"""
                                    ...
                                    """)
        self._datastore.execute_query(f"DROP VIEW public.tmp_1")
    ...
    def get_column_min(self, column_name):
        query = f'SELECT min(a."{column_name}") as type from ({self._query}) AS a limit 1'
        return self._datastore.fetch_one(query)[0]

    def get_column_max(self, column_name):
        query = f'SELECT max(a."{column_name}") as type from ({self._query}) AS a limit 1'
        return self._datastore.fetch_one(query)[0]
